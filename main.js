...
const RepoSearchBarEvent = (e) => {
    ...
    const arrayOfFilteredRepos = arrRepositories.filter(repo => {
        return (repo.repoTitle.includes(searchStringBar) ||
...
const SortRepositoriesBy = (e) => {
    ...
    if (sortReposBy === "srtByID") {
        ...
        if (repoAscendDecend === "srtAscending") {
            arrRepositories.sort((a, b) => (a.repoTitle.toUpperCase() < b.repoTitle.toUpperCase() ? -1 : 1));
        } else {
            arrRepositories.sort((a, b) => (a.repoTitle.toUpperCase() > b.repoTitle.toUpperCase() ? -1 : 1));
        }
    } else if (sortReposBy === "srtBylenguage") {
        if (repoAscendDecend === "srtAscending") {
            arrRepositories.sort((a, b) => (a.repoLanguage.toUpperCase() < b.repoLanguage.toUpperCase() ? -1 : 1));
        } else {
            arrRepositories.sort((a, b) => (a.repoLanguage.toUpperCase() > b.repoLanguage.toUpperCase() ? -1 : 1));
        }
    }
...
const PrintProjectsFormWithSearchBar = () => {
	let projectCardsWithSearchBar = `<div><input type="text" id="projectSearchbar" name="search" onkeyup="projectSearchbar" placeholder="Search projects.." /><div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
...
const searchProjectsSearchBar = (e) => {
	const projectsList = document.querySelector('#projectSearchBar').value.trim();
	
	const arrFilteredProjects = arrProjects.filter(project => {
		return (project.projectName.includes(projectsList)) ||
			project.repoDescription.includes(projectsList)
	});
	PrintProjectsFormWithSearchBar(arrFilteredProjects);
};
